...
ln -sv $WEST_SIM_ROOT/amber_config/P53.prmtop .
...
# Now take care of the case that the trajectory is starting anew.
  ...
  ln -sv $WEST_PARENT_DATA_REF.rst ./parent.rst
fi

############################## Run the dynamics ################################
# Propagate segment using pmemd (or sander)
$PMEMD -O -i md.in   -p P53.prmtop  -c parent.rst \
          -r seg.rst -x seg.nc -o seg.log    -inf seg.nfo

# Copy the trajectory into the location where it will be used by pcoord_loader.py
cp seg.nc $WEST_PCOORD_RETURN

########################## Calculate and return data ###########################
# Clean up
rm -f $TEMP md.in parent.rst seg.nfo seg.pdb P53.prmtop
